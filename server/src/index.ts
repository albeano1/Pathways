...
import { isValidPuzzleHops } from "../../shared/puzzleRules.js";
import type {
  HintResponse,
  ScoreRequest,
  ScoreResponse,
...
import { PuzzleService, difficultyFromHops } from "./puzzles.js";
  ...
  try {
    ...
    if (start && end) {
      ...
      if (!isValidPuzzleHops(optimalHops)) {
        res.status(400).json({
          error: `Path length must be ${3}–${6} hops (got ${optimalHops})`,
        });
        return;
      }
...
app.listen(PORT, () => {
  console.log(
    `Server running at http://localhost:${PORT} (${graph.getWordCount().toLocaleString()} words)`
  );
});
